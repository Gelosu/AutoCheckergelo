...
export default function PresetTest() {
  ...
  const handlePreview = (testpaper) => {
    ...
    setPreviewTitle(
      `Test Previewing - Test Number: ${testpaper.test_number}, Test Name: ${testpaper.test_name} in ${testpaper.class_name}`
    ); // Customize the title format as needed

    // Show the "Preview" modal
    setShowPreviewModal(true);
  };

  const filterTestpapers = (year) => {
    const filtered = testpapers.filter((testpaper) => {
      const testYear = new Date(testpaper.created_at).getFullYear().toString();
      return year === "All" ? true : testYear === year;
    });
    const sorted = [...filtered].sort((a, b) => {
      const year1 = new Date(a.created_at).getFullYear();
      const year2 = new Date(b.created_at).getFullYear();
      return year2 - year1;
    });
    setFilteredTestpapers(sorted);
  };
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  ...
                  <p className="text-start mb-1">
                    SHARING PRESET UID: {selectedUID}
                  </p>
                  {/* Display additional information */}
                  {presetInfo2 && (
                    <div>
                      <p>Additional Information:</p>
                      <pre>{JSON.stringify(presetInfo2, null, 2)}</pre>
                    </div>
                  )}
                </div>
